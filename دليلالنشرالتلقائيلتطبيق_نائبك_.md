...
gcloud run services describe naibak-auth-service --region=us-central1
...
### URLs مهمة بعد النشر
- **الخدمة الرئيسية:** `https://naibak-auth-service-xxx.a.run.app`
- **فحص الصحة:** `https://naibak-auth-service-xxx.a.run.app/health/`
- **توثيق API:** `https://naibak-auth-service-xxx.a.run.app/api/docs/`
- **مقاييس الحوكمة:** `https://naibak-auth-service-xxx.a.run.app/api/ai-governance/metrics/`
...
```bash
# فحص logs الخدمة
gcloud logs read "resource.type=cloud_run_revision AND resource.labels.service_name=naibak-auth-service" --limit=20

# اختبار محلي
docker run -p 8000:8000 gcr.io/your-project/naibak-auth-service:latest
...
gcloud run services update-traffic naibak-auth-service \
    --to-revisions=naibak-auth-service-previous-revision=100 \
    --region=us-central1
...
# فحص صورة Docker
trivy image gcr.io/your-project/naibak-auth-service:latest
```
